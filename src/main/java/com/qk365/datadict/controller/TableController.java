...
import com.qk365.datadict.dto.TableInfoExp;
...
public class TableController {
    ...
    String left(Model model, HttpServletRequest request) {
        ...
        if (dataSourceList1.getType() == 1){
            list = mySqlService.findTableName(dbKey);
        }else if (dataSourceList1.getType() == 2){//sqlserver
            list = sqlServerService.findTableName(dbKey);
        }else if (dataSourceList1.getType() == 3){//sqlite
    ...
    String table(@PathVariable("tablename") String tablename,
       ...
       String dbKey = "oa";
        ...
        List<Map<String, Object>> listTable = sqlServerService.findTableNameExplain(Long.valueOf(id),dbKey);

        String explain = "请输入表说明";
        if (listTable != null && listTable.size() > 0) {
            explain = listTable.get(0).get("explain").toString();
        }
    ...
    ,@RequestParam("dbKey") String dbKey) {
        ...
        for(TableInfo table:list){
            ...
            json.put("width",100);
            /**
             if(table.getDataType().equals("datetime")){
             json.put("templet", "");
             }
             */
            array.add(json);
    ...
    public Object applyAjaxList(QueryInfoDto query) {
        try {
//			List<Map<String,Object>>  list =commonService.findList(query);
            PaginationSupport result = commonService.queryAjaxList(query);
